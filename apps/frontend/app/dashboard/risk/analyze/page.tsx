...
import { RECHARTS_GRID_STROKE, RECHARTS_TICK } from '@/lib/chart-theme';
      ...
      <Tabs defaultValue="factors" className="w-full">
                ...
                <div className="h-[min(400px,55vh)] w-full min-h-[300px] overflow-visible">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      ...
                      barGap={2}
                    >
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={RECHARTS_GRID_STROKE} />
                      <XAxis
                        dataKey="factor"
                        interval={0}
                        angle={0}
            ...
            <CardContent className="space-y-3">
              {dynamicInsights.map((insight, idx) => (
                <div key={idx} className="flex gap-3">
                  ...
                  <p className="text-sm text-foreground/80 dark:text-zinc-200">{insight}</p>
                </div>
              ))}
            </CardContent>
          </Card>
        ...
        <TabsContent value="correlation" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-foreground/80 dark:text-zinc-200">
                {dynamicCorrelationSummary}
              </p>
              <div className="bg-secondary p-4 rounded-lg space-y-2">
                <p className="text-sm font-medium">{t('risk.analyze.strong_indicators')}</p>
                <ul className="text-sm text-foreground/80 dark:text-zinc-200 space-y-1">
