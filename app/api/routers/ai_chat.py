...
def get_chat_service() -> AIChatService:
    ...
    if provider == "openai":
        try:
            from app.services.openai_ai_chat_service import OpenAIChatService

            return OpenAIChatService()
        except Exception as e:
            if _fallback_to_mock_enabled() and not _looks_like_auth_or_config_error(e):
                logger.warning("OpenAI init failed (%s) -> fallback to mock", str(e))
                return MockAIChatService()
            raise

    if provider == "langflow":
        try:
            from app.services.langflow_ai_chat_service import LangflowAIChatService

            return LangflowAIChatService()
        except Exception as e:
            if _fallback_to_mock_enabled() and not _looks_like_auth_or_config_error(e):
                logger.warning("Langflow init failed (%s) -> fallback to mock", str(e))
                return MockAIChatService()
            raise
...
async def ai_chat_debug(current_user: User = Depends(get_current_active_user)):
    ...
    return {
        ...
        "langflow_run_url": (os.getenv("LANGFLOW_RUN_URL") or "").strip() or None,
        "langflow_base_url": (os.getenv("LANGFLOW_BASE_URL") or "").strip() or None,
        "langflow_flow_id": (os.getenv("LANGFLOW_FLOW_ID") or "").strip() or None,
    }
