...
from sqlalchemy import Boolean, Column, Date, DateTime, Numeric, ForeignKey, Integer, String, Text, BigInteger, UnicodeText, UniqueConstraint
...
class UserDB(Base):
    ...
    manager_upgrade_requests = relationship(
        "ManagerUpgradeRequestDB",
        back_populates="target_user",
        foreign_keys="ManagerUpgradeRequestDB.target_user_id",
    )
    manager_upgrade_nominations = relationship(
        "ManagerUpgradeRequestDB",
        back_populates="nominated_by_user",
        foreign_keys="ManagerUpgradeRequestDB.nominated_by",
    )
    manager_upgrade_votes = relationship(
        "ManagerUpgradeVoteDB",
        back_populates="manager_user",
        foreign_keys="ManagerUpgradeVoteDB.manager_user_id",
    )


# ============================================================================
# Manager Upgrade Request (Analyst -> Manager workflow)
# ============================================================================
class ManagerUpgradeRequestDB(Base):
    __tablename__ = "Manager_Upgrade_Request"

    request_id = Column(BigInteger, primary_key=True, autoincrement=True)
    target_user_id = Column(BigInteger, ForeignKey("User.user_id"), nullable=False)
    purpose = Column(String(1000), nullable=False)
    status = Column(String(20), nullable=False, default="pending")  # pending, approved, rejected
    requested_by_role = Column(String(20), nullable=False, default="analyst")  # analyst or manager
    nominated_by = Column(BigInteger, ForeignKey("User.user_id"), nullable=True)  # manager nominator
    approved_by = Column(BigInteger, ForeignKey("User.user_id"), nullable=True)  # admin/manual final approver
    approved_at = Column(DateTime, nullable=True)
    rejection_reason = Column(String(1000), nullable=True)
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at = Column(DateTime, nullable=True)

    target_user = relationship("UserDB", foreign_keys=[target_user_id], back_populates="manager_upgrade_requests")
    nominated_by_user = relationship("UserDB", foreign_keys=[nominated_by], back_populates="manager_upgrade_nominations")
    votes = relationship("ManagerUpgradeVoteDB", back_populates="request", cascade="all, delete-orphan")


class ManagerUpgradeVoteDB(Base):
    __tablename__ = "Manager_Upgrade_Vote"
    __table_args__ = (
        UniqueConstraint("request_id", "manager_user_id", name="uq_manager_upgrade_vote_request_manager"),
    )

    vote_id = Column(BigInteger, primary_key=True, autoincrement=True)
    request_id = Column(BigInteger, ForeignKey("Manager_Upgrade_Request.request_id"), nullable=False)
    manager_user_id = Column(BigInteger, ForeignKey("User.user_id"), nullable=False)
    vote = Column(String(20), nullable=False)  # approve, reject
    note = Column(String(1000), nullable=True)
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)

    request = relationship("ManagerUpgradeRequestDB", back_populates="votes")
    manager_user = relationship("UserDB", foreign_keys=[manager_user_id], back_populates="manager_upgrade_votes")
